...
export default function MfgSidebar() {
  ...
  return (
    <div className="flex flex-col w-64 bg-slate-950 border-r border-violet-900/30 h-screen text-slate-300">
      <div className="flex flex-col items-center justify-center h-20 border-b border-slate-800 px-4">
        <h1 className="text-xl font-bold text-white tracking-wider">Smart<span className="text-violet-500">ERP</span></h1>
        <span className="text-xs text-violet-400 mt-0.5 font-medium">Manufacturing Portal</span>
      </div>
          ...
          {nav.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors group ${active ? 'bg-violet-500/20 text-violet-300' : 'hover:bg-slate-900 hover:text-white'}`}>
                <item.icon className={`mr-3 h-5 w-5 ${active ? 'text-violet-400' : 'text-slate-500 group-hover:text-violet-400'}`} />
                {item.name}
              </Link>
            );
          })}
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center mb-4">
          ...
          <div className="ml-3">
            <p className="text-sm font-medium text-white">Manufacturing Manager</p>
